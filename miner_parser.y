...
Struct *output; // the parsed struct
char *identifier; // the identifier for top level struct (eg. Life)
...
extern "C"
{
    ...
    void switch_string(const char *str); // in miner_parser.l
}
...
/* union to store result returned from rules */
%union {
    int int_value;
    char *string_value;
...
/* tell bison how to store data generated by rules */
...
/* reset status for the state machine */
%start file
...
/* string, trim double quotemarks returned by flex */
string : TOKEN_STRING {
           string tmp($1);
           tmp.erase(0, 1);
 ...
 * this method is NOT threaded safe because we are using global variables
 * to sture the result
 */
